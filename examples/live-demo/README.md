...
### 7. Evaluate the policy gate

```bash
agentscope gate
```

The gate reads the evidence, recomputes the risk score, and evaluates the
effective gate policy. For this scenario it **fails** (exit code `1`) because the
session had a blocked-path deny and the deny count exceeds the default
threshold. `agentscope gate --json` prints the full `GateResultV1`.

### 8. Generate a CI summary

```bash
agentscope ci-summary --output .agentscope/ci/summary.md
```

This writes a human-readable Markdown summary of the task, scope hash, risk
score, denied / asked / high-risk actions, top risk factors, and
recommendations. It is **display only** — it never changes the gate's exit code.
...
- [`expected-gate-result.json`](expected-gate-result.json) — example `agentscope gate --json` output (`status: fail`). Paths and timestamps use `<project>` / `<timestamp>` placeholders.
- [`expected-ci-summary.md`](expected-ci-summary.md) — example `agentscope ci-summary` Markdown output.
- [`demo-script.md`](demo-script.md) — a 30–60s narration + command script for recording a GIF or screencast.

Your exact ids, timestamps, and paths will differ, but the decisions, factors, score, gate result, and summary should match.
